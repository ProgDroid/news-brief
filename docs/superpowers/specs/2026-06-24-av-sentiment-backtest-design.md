...
- **Rate-limit discipline:** free tier = **25 req/day, 5/min**. The puller does at most N calls per invocation (default ≤ 25), sleeps to respect 5/min, and **persists a manifest** (`manifest.json`) of completed `(ticker, kind, cursor)` units so re-running resumes exactly where it stopped — designed to be run once a day across several days until the universe is complete. Detect AV's throttle response (a JSON body with an `Information`/`Note` key) and stop cleanly, recording remaining work.
...
3. **Transcript fiscal-quarter labeling** — AV's `quarter` param maps to fiscal quarters (e.g. MU `2024Q1` = the call reported Dec 2023); the transform dates points by the transcript's actual call date, not the label, so fiscal/calendar mismatch doesn't distort alignment.
4. **Free-tier pull duration** — ~37 names × (news pages + ~60 transcript quarters) at 25/day spans roughly one to two weeks; acceptable per the "spread over days" decision. Operator may opt into one month of premium ($49.99) to compress to minutes if impatient.
